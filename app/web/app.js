...
async function importJsonFile(event) {
  ...
  try {
    ...
    const request = parsed.input || parsed;
    if (!request.scenario_id || !Array.isArray(request.hours)) throw new Error('Not a GridWise request');
    ui.payload.value = JSON.stringify(request,null,2);
    updatePayloadState();
    clearResult(request.scenario_id);
...
function clearResult(title) {
  ...
  ui.chart.innerHTML = '<div class="empty-state">Run this custom request to see the optimized dispatch.</div>';
  ui.directives.innerHTML = '<div class="empty-state compact">Interpretations will appear after optimization.</div>';
  ui.directiveCount.textContent = '0';
  ui.validation.innerHTML = '<li><span>○</span> Awaiting custom schedule</li>';
  ui.scheduleBody.innerHTML = '<tr><td colspan="6" class="empty-cell">Run the custom input to create a schedule.</td></tr>';
