...
- Browser dashboard at `/` with sample loading, JSON editing, live API execution, dispatch chart, directive inspection, replay checks, and result download.
...
- The solver result is replayed before it is returned.
- Totals are recalculated from the final hourly plan.
...
```text
app/          API, LLM interpreter, guardrail, optimizer, and dashboard
tests/        unit, schema, API, error, and official-cost tests
samples/      canonical ten-case public pack
...
Before final submission, add the real deployed base URL, the public-after-deadline repository URL, the exact pullable image tag/digest, and the accessible video link to the organizer form. Do not commit any secret value.
